...
func TestPrepareBeaconProposer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "Happy Path",
			request: []*shared.FeeRecipient{{
				FeeRecipient:   "Qb698D697092822185bF0311052215d5B5e1F393400000000000000000000000000000000000000000000000000000000",
				ValidatorIndex: "1",
			},
			},
			code:    http.StatusOK,
...
`
	...
	invalidBeaconCommitteeContribution = `[
  {
	...
	registrations = `[{
    "message": {
      "fee_recipient": "Qabcf8e0d4e9587369b2301d0790347320302cc0900000000000000000000000000000000000000000000000000000000",
      "gas_limit": "1",
      "timestamp": "1",
